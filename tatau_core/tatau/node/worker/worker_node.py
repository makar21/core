...
class Worker(Node):
    ...
    def _process_task_assignment(self, task_assignment):
        ...
        if task_assignment.state == TaskAssignment.State.DATA_IS_READY:
            ...
            work_process = Process(
                target=self._work,
                args=(task_assignment.asset_id, metrics_collector),
            )
            work_process.start()
            work_process.join()
    ...
    def _work(self, asset_id, metrics_collector):
        ...
        try:
            ...
            progress = TaskProgress(self, asset_id, metrics_collector)
            ...
            task_assignment.tflops = metrics_collector.get_tflops()
