...
class Dataset(models.Model):
    ...
    def download_and_create(cls, x_train_url, y_train_url, x_test_url, y_test_url, minibatch_size, **kwargs):
        ...
        try:
            ...
            kwargs['test_dir_ipfs'] = ipfs.add_dir(test_target_dir, recursive=True).multihash
            logger.info('Test part is uploaded: {}'.format(kwargs['test_dir_ipfs']))
            ...
            for batch_idx in range(0, batches):
                ...
                x_path = os.path.join(train_dir, 'x_train_' + name_format.format(batch_idx))
                np.save(x_path, x_batch)
                y_path = os.path.join(train_dir, 'y_train_' + name_format.format(batch_idx))
